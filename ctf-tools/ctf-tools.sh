...
install_rev_tools () {
    ...
    if [ -d "$REV_TOOL_PATH/$GHIDRA_VERSION" ]; then
        ...
        wget https://github.com/NationalSecurityAgency/ghidra/releases/download/Ghidra_10.1.5_build/ghidra_10.1.5_PUBLIC_20220726.zip -O /tmp/ghidra.zip -q
    ...
    installing "IDA"
    IDA_VERSION="idafree-8.1"
    if [ -d "$REV_TOOL_PATH/$IDA_VERSION" ]; then
        skipping
    else
        wget https://out7.hex-rays.com/files/idafree81_linux.run -O /tmp/ida.run -q
        chmod +x /tmp/ida.run
